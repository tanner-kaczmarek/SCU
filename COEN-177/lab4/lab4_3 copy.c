...
int main(){
    ...
    //doing the threads in two steps
    /*
    for(p = 0; p < half; p++)   //only the first half of the threads for matrix multiplication
        pthread_create(&threads[p], NULL, matMuptiply, (void *)p);  //creates threads for each column of the first half
    for(p = 0; p < half; p++)
    ...
    for(p = half; p < SIZE; p++)  //do the second half of the threads for matrix multiplication
        pthread_create(&threads[p], NULL, matMuptiply, (void *)p);  //creates threads for each column of the second half
    for(p = half; p <SIZE; p++)
        pthread_join(threads[p],NULL);   //it will wait until the thread has finished before terminating it.  This ensures every column is filled.
     */
