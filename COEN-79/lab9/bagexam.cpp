    ...
    )
{
    ...
    if (test.size( ) != n){
        answer = false;
        cout << "Size comparison failed. Expected: " << n << " Received: " << test.size() << ".\n";
    }
    else
        for (i = 0; /*answer &&*/ (i <= max); i++)//before commenting out "answer &&" the test would end at the first incorrect count. This way, it lists every wrong count, and can help detect when a subtree goes missing.
            if (count[i] != test.count(i)){
                answer = false;
                cout << "Count comparison failed. Expected: " << count[i] << " " << i << "'s. Received: " << test.count(i) << " " << i << "'s.\n";//improved print statements to include more information.
            }
...
int test1( )
{
    ...
    cout << test_letter++ << ". ";    cout << "Testing size and count for an empty bag.";
    cout << endl;
